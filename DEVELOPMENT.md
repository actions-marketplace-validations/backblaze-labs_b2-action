...
Every third-party action under `.github/workflows/` is pinned to a full commit SHA with a trailing exact-version comment (for example `uses: actions/checkout@<sha> # v6.0.2`), so a moved or compromised upstream tag cannot run in our CI or the `contents: write` release job. The comment names the precise release the SHA represents, so a reviewer can confirm it at a glance. Dependabot's `github-actions` updates bump the SHA and the comment together. When you add a workflow step, pin it the same way: resolve the tag with `gh api repos/<owner>/<repo>/commits/<tag> -q .sha` and add the `# vX.Y.Z` comment. The repo's own action is referenced as `uses: ./` and is not pinned. This is enforced automatically: `pnpm lint:actions` ([`scripts/check-action-pins.mjs`](./scripts/check-action-pins.mjs)) runs in the pre-commit hook and the CI `actionlint` job and fails on any third-party action that is not SHA-pinned, so an accidental regression to `@v1` cannot merge.
