...
- Add a dependency vulnerability gate: a CI `audit` job runs `pnpm audit --prod --audit-level high` (also `pnpm run audit`), failing the build on a high or critical advisory in a production dependency. Scoped to production deps so a dev-tooling advisory cannot block an unrelated PR. ([#21](https://github.com/backblaze-labs/b2-action/issues/21))
